...
for (const m of text.matchAll(rowRe)) {
  legacy.push({ legacy_id: m[1], surah: Number(m[2]), ayah: Number(m[3]), token: Number(m[4]), family: m[5], authority: m[6] });
}
if (legacy.length !== 169) throw new Error(`Expected 169 legacy rows, found ${legacy.length}`);

const head = [
  ['2:17:1','nominal_mathal'], ['2:17:2','nominal_mathal'], ['2:26:7','nominal_mathal'],
  ['2:26:28','nominal_mathal'], ['2:171:1','nominal_mathal'], ['2:171:4','nominal_mathal'], ['2:214:8','nominal_mathal']
];
const tail = [
  ['66:11:3','nominal_mathal'], ['74:31:38','nominal_mathal'], ['76:28:8','nominal_mathal'], ['6:38:12','noun_amthal']
];
const parseLoc = (s) => { const [surah,ayah,token] = s.split(':').map(Number); return {surah,ayah,token}; };
const key = (r) => `${r.surah}:${r.ayah}:${r.token}`;

// QSIM-000003 authority: 7 true head rows + legacy QLO-000001..000077 shifted by +7,
// excluding four duplicated head locators + 4 restored tail rows + legacy QLO-000089..000169 preserved.
...
const locFamily = new Set(canonical.map(r=>r.canonical_row_key));
if (ids.size !== 169 || locFamily.size !== 169) throw new Error('Uniqueness gate failed');

// Wave 2 candidate generation is deliberately broad: lexical anchors become reviewable proposals,
// never parable decisions. Same-verse anchors cluster into one proposal.
const groups = new Map();
for (const r of canonical) {
  if (r.family !== 'nominal_mathal') continue;
  const gk = `${r.surah}:${r.ayah}`;
...
const candidates = [...groups.entries()].map(([verse, anchors], i) => ({
  ...
  discovery_channel: 'LEXICAL_STRUCTURAL_ANCHOR',
  verse,
  initial_boundary: verse,
  anchor_occurrence_ids: anchors.map(a=>a.occurrence_id),
  anchor_locators: anchors.map(a=>a.source_locator),
