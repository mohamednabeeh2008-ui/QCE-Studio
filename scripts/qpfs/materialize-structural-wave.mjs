...
// QSIM-000003 is the only identity authority here:
// 7 materialized head + legacy 12..88 mapped to canonical 8..84 + 4 restored tail + legacy 89..169 preserved.
const canonical = [];
for (const [source_locator,family] of head) canonical.push({...parseLoc(source_locator),family,authority:'QSIM-000003:MATERIALIZE_HEAD'});
for (const r of legacy.slice(11,88)) canonical.push({...r,authority:`QSIM-000003:COMPACT_EXISTING_NOMINAL_RANGE:${r.authority}`});
for (const [source_locator,family] of tail) canonical.push({...parseLoc(source_locator),family,authority:'QSIM-000003:RECOVER_TERMINAL_NOMINAL_ROWS'});
for (const r of legacy.slice(88)) canonical.push({...r,authority:`QSIM-000003:PRESERVE_NON_NOMINAL_RANGE:${r.authority}`});
if (canonical.length !== 169) throw new Error(`QSIM arithmetic failed: ${canonical.length}`);
canonical.forEach((r,i)=>{r.occurrence_id=`QLO-${String(i+1).padStart(6,'0')}`;r.source_locator=loc(r);r.canonical_row_key=`${r.source_locator}|${r.family}`;});
const ids = new Set(canonical.map(r=>r.occurrence_id));
const rowKeys = new Set(canonical.map(r=>r.canonical_row_key));
if (ids.size !== 169 || rowKeys.size !== 169) throw new Error('Canonical uniqueness gate failed');
...
const report={
  ...
  root_blind:{status:'ROUTED_TO_CORPUS_WIDE_DISCOVERY',reason:'LEXICAL_169_ROW_INTAKE_CANNOT_PROVE ROOT_BLIND COMPLETENESS'},
...
console.log(`PASS: Wave 2 reconciled 169 rows, routed ${candidates.length} unresolved candidate proposals, created no parable decisions and no QPUs`);