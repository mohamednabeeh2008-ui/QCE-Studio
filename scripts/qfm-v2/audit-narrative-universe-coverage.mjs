...
for(const file of files){
  ...
  if(ids.length||lanes.length) rows.push({source:rel,sha256:crypto.createHash('sha256').update(text).digest('hex'),ids,lanes});
}
...
const classifySource=source=>{
  ...
  if(/permanent-narrative-registry[^/]*\.jsonl$/.test(s)) return 'ALLOCATION_AUTHORITY';
  if(/hold|quarantine|blocked/.test(s)) return 'HOLD_EVIDENCE';
  if(/promotion|promot|migration|upgrade/.test(s)) return 'PROMOTION_EVIDENCE';
  if(/production|produced|batch|ledger|registry/.test(s)) return 'PRODUCTION_EVIDENCE';
...
const narReconciliation=expectedPermanentNarIds.map(nar_id=>{
  const sources=rows.filter(r=>r.ids.includes(nar_id)).map(r=>({source:r.source,sha256:r.sha256,evidence_class:classifySource(r.source),lanes:r.lanes}));
  sources.sort((a,b)=>evidenceRank[b.evidence_class]-evidenceRank[a.evidence_class]||a.source.localeCompare(b.source));
  const classes=[...new Set(sources.map(s=>s.evidence_class))];
  const status=classes.includes('ALLOCATION_AUTHORITY')?'AUTHORITY_PROVEN':classes.includes('PRODUCTION_EVIDENCE')?'PRODUCTION_PROVEN':classes.includes('PROMOTION_EVIDENCE')?'PROMOTION_PROVEN':classes.includes('HOLD_EVIDENCE')?'HOLD_EVIDENCE_ONLY':sources.length?'REFERENCE_ONLY':'MISSING';
...
const narReconciliationManifest={
  schema:'qfm-v2/nar-evidence-reconciliation/v0.1',
  generated_by:'scripts/qfm-v2/audit-narrative-universe-coverage.mjs',
  scope:{first:'NAR-000001',last:'NAR-000038',count:38},
  rules:{promotion_requires_evidence_chain:true,reference_is_not_allocation:true,authority_rule:'Only permanent-narrative-registry*.jsonl files prove permanent NAR allocation.',classification_precedence:['ALLOCATION_AUTHORITY','PRODUCTION_EVIDENCE','PROMOTION_EVIDENCE','HOLD_EVIDENCE','REFERENCE_ONLY']},
  counts:reconciliationCounts,
  unresolved_authority_ids:narReconciliation.filter(r=>!r.authority_proven).map(r=>r.nar_id),
  records:narReconciliation
};

const provenance={
  observed_nar_universe:{status:missingObservedPermanentNarIds.length===0?'CONTIGUOUS_REFERENCE_COVERAGE':'REFERENCE_GAPS_PRESENT',scope:'Repository-observed NAR references only; references do not by themselves prove permanent allocation.',expected_range:['NAR-000001','NAR-000038'],observed_ids:observedPermanentNarIds,missing_ids:missingObservedPermanentNarIds},
  authoritative_nar_allocations:{status:referencedButNotAuthorityAllocatedNarIds.length===0?'FULLY_RECONCILED':'RECONCILIATION_REQUIRED',authority_rule:'Only permanent-narrative-registry*.jsonl files prove permanent NAR allocation.',allocated_ids:allocatedNarIds,observed_but_not_authority_allocated_ids:referencedButNotAuthorityAllocatedNarIds},
  cid_corpus:{status:cidIds.length===0?'NOT_MATERIALIZED_IN_SCANNED_SOURCE_ROOTS':'OBSERVED_IN_REPOSITORY',scope:'No historical CID count is inferred or fabricated. Recovery requires repository evidence or an explicit canonical manifest.',observed_ids:cidIds,scanned_source_roots:allowedRoots}
};

const out={
  schema:'qfm-v2/narrative-universe-source-census/v0.3',generated_by:'scripts/qfm-v2/audit-narrative-universe-coverage.mjs',source_roots:allowedRoots,source_file_count:files.length,evidence_file_count:rows.length,
  ...
  provenance,nar_reconciliation_summary:{counts:reconciliationCounts,unresolved_authority_count:narReconciliationManifest.unresolved_authority_ids.length},
  mission_gate:{mission:'MISSION 005-PRE',complete:false,blockers:[...(missingObservedPermanentNarIds.length?['NAR_REFERENCE_GAPS']:[]),...(referencedButNotAuthorityAllocatedNarIds.length?['NAR_ALLOCATION_RECONCILIATION_REQUIRED']:[]),...(cidIds.length===0?['CID_CORPUS_NOT_MATERIALIZED']:[])]},
  invariants:{last_nar_id_expected:'NAR-000038',next_nar_id_expected:'NAR-000039',nar_000039_must_not_be_allocated:true},
  safety:{nar_000039_referenced:nar000039ReferenceRows.length>0,nar_000039_reference_rows:nar000039ReferenceRows,nar_000039_allocated:nar000039AllocationRows.length>0,nar_000039_allocation_rows:nar000039AllocationRows,allocation_authority_rule:'Only permanent-narrative-registry*.jsonl files prove permanent NAR allocation.',pass:nar000039AllocationRows.length===0},
  evidence:rows
};
...
console.log(`NAR RECONCILIATION: ${Object.entries(reconciliationCounts).map(([k,v])=>`${k}=${v}`).join(' ')} unresolved_authority=${narReconciliationManifest.unresolved_authority_ids.length}`);
